...
2. 在“基础行为”中选择工作模式。第一次使用建议选“自动：优化并上传”。
3. 展开“上传服务”，选择实际使用的服务商并按照内置说明填写。
4. 点击“测试连接”。
5. 在临时笔记中粘贴一张图片，确认生成的远程地址可以在无痕浏览器中打开。
...
| 模式 | 粘贴/拖入时的行为 | 适合人群 |
|---|---|---|
| 自动 | 优化、上传、验证并插入远程链接 | 已配置对象存储，希望无感使用 |
| 半自动 | 优化后保存为本地附件 | 暂不上传，或希望之后批量迁移 |
| 手动 | 不接管粘贴和拖拽，只响应命令 | 希望完全控制处理时机 |
...
| 字段 | 含义 |
|---|---|
| `image-upload: false` | 自动粘贴、拖拽和批量迁移跳过该笔记 |
