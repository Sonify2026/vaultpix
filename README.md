...
VaultPix 是一个面向 Obsidian Desktop 的图片工作流插件：粘贴或拖入图片后，它可以保留原图或按需优化尺寸与格式、按模板命名、上传到对象存储、验证公开地址、替换 Markdown 链接，并以可恢复的方式处理本地原图。
...
- 图片处理：新安装默认原图直通，不重编码、不缩放；也可主动选择 WebP、JPEG、PNG、AVIF 和缩放策略。
...
1.3.0 起，新安装默认保持原图字节，避免截图文字因有损转码或缩小而变模糊。已有安装会保留之前选定的 WebP、画质和尺寸设置，不会在升级时擅自改变；若需要完全保真，请在“图片优化”中选择“保持原格式与清晰度”。该模式只命名与保存，不执行缩放，也不会通过重编码减小文件体积。选择 WebP、JPEG、AVIF 时可能产生有损画质变化；PNG 编码无损，但缩小尺寸仍会减少像素。
...
- 本地批量处理会创建按当前设置处理的新附件并更新引用，为安全起见保留原图片。
